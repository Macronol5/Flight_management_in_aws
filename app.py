...
def signup():
    if request.method == 'POST' and 'email' in request.form and 'password' in request.form and 'name' in request.form:
        ...
        with connection.cursor() as cursor:
            ...
            if check:
                return render_template('signlog.html', found2='User Already exists')
            else:
...
def reasult():
    if 'email' in session:
        ...
        return render_template('reasult.html', right=right, interval=interval, wrong=wrong,
                               total=(right + wrong), name=session['name'])
    return '<h1>Please Login First</h1>'
...
def forget_password():
    if 'email' in session:
        return 'You are already logged in; sign out first'
    else:
        if request.method == 'POST':
            ...
            with connection.cursor() as cursor:
                ...
                if detail:
                    ...
                    msg = Message(subject='Reset Password',
                                  body='Hello ' + str(detail[0]['name']) + ' your password is ' +
                                       str(detail[0]['password']) + ' . Thanks For using our services.',
